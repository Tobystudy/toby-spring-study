...
#### 싱글톤 패턴
싱글톤 패턴은 어떤 클래스를 어플리케이션 내에서 제한된 인스턴스 개수, 주로 1개만 존재하도록 강제하는 패턴.<br>
어플리케이션 내에서 전역적으로 접근이 가능한 장점이 있다.<br>
싱글톤 패턴으로 빈을 만드는 이유 -> 엔터프라이즈 기술을 사용하는 서버환경 즉 많은 요청이 오기에 매번 새로 오브젝트를 만들면 감당하기 힘들다.
#### 자바에서 싱글톤을 구현하는 방법###
- 클래스 밖에서 오브젝트를 생성하지 못하도록 생성자를 private로 만든다.
- 생성된 싱글톤 오브젝트를 저장할 수 있는 같은 타입의 스태틱 필드를 정의한다.
- 스태틱 팩토리 메소드인 getInstance()를 만들고 최초 호출 시점에 오브젝트를 만들어 두거나 필드의 초기값으로 오브젝트를 미리 만들어 둔다.
- 한번 오브젝트가 만들어지고 난 후에는 getInstance()메소드를 통해 이미 만들어진 스태틱 필드에 저장해둔 오브젝트를 넘겨준다.
